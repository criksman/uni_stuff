...
/*update prestamo
    set
    fecha_termino = sysdate + 5, fecha_devolucion = null, estado = 'P';
where rut_socio in (select rut_socio from socio where tipo = 'A') and fecha_inicio in (to_date('10/11/2019'), to_date('11/11/2019'), to_date('12/11/2019')) and fecha_termino - fecha_inicio > 3 and cod_copia in (select c.cod_copia from copia c, titulo t where c.cod_titulo = t.cod_titulo and t.descripcion like '%AMOR%');*/
